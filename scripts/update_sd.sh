...
REPO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)/stable-diffusion.cpp"
INSTALL_DIR="$(cd "$SCRIPT_DIR/.." && pwd)/install_sd"
...
do_vulkan() { check_vulkan; build_backend "$REPO_DIR" "$INSTALL_DIR" vulkan -DSD_VULKAN=ON; }
do_nvidia() { check_nvidia; build_backend "$REPO_DIR" "$INSTALL_DIR" nvidia -DSD_CUDA=ON; }
...
show_menu() {
    echo "" >&2
    echo "Select stable-diffusion.cpp backend(s) to build (space separated, e.g. '1 3'):" >&2
    echo "  1. cpu" >&2
    echo "  2. vulkan" >&2
    echo "  3. nvidia" >&2
    echo -n "> " >&2
...
main() {
    sync_repo "$REPO_DIR" "https://github.com/leejet/stable-diffusion.cpp" "--recursive"
    if [ "$#" -gt 0 ]; then
